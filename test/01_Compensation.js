...
describe("Compensation", function () {
    ...
    describe('Transactions', async () => {
        it('check data', async () => {
            ...
            expect(amount1.toString()).to.be.equal('0');

            let amount2 = await compensation.calcClaimAmount(accounts[1].address);
            expect(amount2.toString()).to.be.equal('0');
            ...
            let halfLastApyBlockTimestamp = compensation_startTimestamp + 15;
            await compensation.setBlockTimestamp(halfLastApyBlockTimestamp);
            ...
            let additionalAmount = '40'; // 15 sec with apy, 1 * 342000000 * 0.25 / 2102400 = 40,6
            ...
            additionalAmount = '41'; // 30 sec with apy = 2 * 342000000 * 0.25 / 2102400 = 81,2 - 40 (claimed)
            expect(stableAcc0BalanceAfter.sub(stableAcc0BalanceBefore).toString()).to.be.equal(additionalAmount);
            expect(stableRefundContractBalanceBefore.sub(stableRefundBalanceAfter).toString()).to.be.equal(additionalAmount);
            ...
            additionalAmount = '132'; // 30 sec with apy = 2 * 558000000 * 0.25 / 2102400 = 132
            expect(stableAcc1BalanceAfter.sub(stableAcc1BalanceBefore).toString()).to.be.equal('558000132');
            expect(stableRefundContractBalanceBefore.sub(stableRefundBalanceAfter).toString()).to.be.equal('558000132');
            ...
            await compensation.setBlockTimestamp(compensation_endTimestamp + 1);
            ...
            ).to.be.revertedWith(revertMessages.compensationCompensationYouCanConvertPTokensBeforeStartTimestampOnly);
        });
    });
